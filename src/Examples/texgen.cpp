...
void makeStripeImageTexGen(void)
{
   int j;
...
void initTexGen(void)
{
   glClearColor (0.0, 0.0, 0.0, 0.0);
   glEnable(GL_DEPTH_TEST);
   glShadeModel(GL_SMOOTH);

   makeStripeImageTexGen();
...
void reshapeTexGen(int w, int h)
{
...
void keyboardTexGen (unsigned char key, int x, int y)
{
   switch (key) {
      case 'e':
      case 'E':
...
int mainTexGen(int argc, char** argv)
{
   ...
   initTexGen ();
   glutDisplayFunc(displayTexGen);
   glutReshapeFunc(reshapeTexGen);
   glutKeyboardFunc(keyboardTexGen);
   glutMainLoop();
   return 0;
}
