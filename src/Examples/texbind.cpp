...
void makeCheckImagesTexBind(void)
{
	int i, j, c;
...
void initTexBind(void)
{
	glClearColor (0.0, 0.0, 0.0, 0.0);
	glShadeModel(GL_FLAT);
	glEnable(GL_DEPTH_TEST);

	makeCheckImagesTexBind();
...
void reshapeTexBind(int w, int h)
{
...
void keyboardTexBind(unsigned char key, int x, int y)
{
	switch (key) {
		case 27:
		exit(0);
...
int mainTexBind(int argc, char** argv)
{
	...
	initTexBind();
	glutReshapeFunc(reshapeTexBind);
	glutDisplayFunc(displayTexBind);
	glutKeyboardFunc (keyboardTexBind);
	glutMainLoop();
	return 0;
}
#else
