...
void makeCheckImagesTexSub(void)
{
   int i, j, c;
...
void initTexSub(void)
{    
   glClearColor (0.0, 0.0, 0.0, 0.0);
   glShadeModel(GL_FLAT);
   glEnable(GL_DEPTH_TEST);

   makeCheckImagesTexSub();
...
void reshapeTexSub(int w, int h)
{
...
void keyboardTexSub (unsigned char key, int x, int y)
{
   switch (key) {
      case 's':
      case 'S':
...
int mainTexSub(int argc, char** argv)
{
   ...
   initTexSub();
   glutDisplayFunc(displayTexSub);
   glutReshapeFunc(reshapeTexSub);
   glutKeyboardFunc(keyboardTexSub);
   glutMainLoop();
   return 0; 
}
#else
