...
void renderTeapotDof(GLfloat x, GLfloat y, GLfloat z, GLfloat ambr,
...
void displayDof(void) {
	...
	for (jitter = 0; jitter < 8; jitter++) {
		...
		accPerspective(45.0, (GLdouble) viewport[2] / (GLdouble) viewport[3],
				1.0, 15.0, 0.0, 0.0, 0.33 * j8[jitter].x, 0.33 * j8[jitter].y,
				5.0);

		/*	ruby, gold, silver, emerald, and cyan teapots	*/
		renderTeapotDof(-1.1, -0.5, -4.5, 0.1745, 0.01175, 0.01175, 0.61424,
				0.04136, 0.04136, 0.727811, 0.626959, 0.626959, 0.6);
		renderTeapotDof(-0.5, -0.5, -5.0, 0.24725, 0.1995, 0.0745, 0.75164,
				0.60648, 0.22648, 0.628281, 0.555802, 0.366065, 0.4);
		renderTeapotDof(0.2, -0.5, -5.5, 0.19225, 0.19225, 0.19225, 0.50754,
				0.50754, 0.50754, 0.508273, 0.508273, 0.508273, 0.4);
		renderTeapotDof(1.0, -0.5, -6.0, 0.0215, 0.1745, 0.0215, 0.07568,
				0.61424, 0.07568, 0.633, 0.727811, 0.633, 0.6);
		renderTeapotDof(1.8, -0.5, -6.5, 0.0, 0.1, 0.06, 0.0, 0.50980392,
...
void reshapeDof(int w, int h) {
	glViewport(0, 0, (GLsizei) w, (GLsizei) h);
}

void keyboardDof(unsigned char key, int x, int y) {
	switch (key) {
	case 27:
		exit(0);
		break;
 ...
 */
int mainDof(int argc, char** argv) {
	glutInit(&argc, argv);
	...
	initDof();
	glutReshapeFunc(reshapeDof);
	glutDisplayFunc(displayDof);
	glutKeyboardFunc(keyboardDof);
	glutMainLoop();
	return 0;
}
