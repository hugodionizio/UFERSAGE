...
void reshapeUnProject(int w, int h) {
...
void mouseUnProject(int button, int state, int x, int y) {
	GLint viewport[4];
...
void keyboardUnProject(unsigned char key, int x, int y) {
	switch (key) {
	case 27:
		exit(0);
		break;
...
int mainUnProject(int argc, char** argv) {
	...
	glutDisplayFunc(displayUnProject);
	glutReshapeFunc(reshapeUnProject);
	glutKeyboardFunc(keyboardUnProject);
	glutMouseFunc(mouseUnProject);
	glutMainLoop();
	return 0;
}
