...
void CALLBACK errorCallbackQuadric(GLenum errorCode) {
...
void initQuadric(void) {
	...
	qobj = gluNewQuadric();
	gluQuadricCallback(qobj, GLU_ERROR, (_GLUfuncptr) errorCallbackQuadric);
...
void reshapeQuadric(int w, int h) {
...
void keyboardQuadric(unsigned char key, int x, int y) {
	switch (key) {
	case 27:
		exit(0);
		break;
...
int mainQuadric(int argc, char** argv) {
	glutInit(&argc, argv);
	...
	initQuadric();
	glutDisplayFunc(displayQuadric);
	glutReshapeFunc(reshapeQuadric);
	glutKeyboardFunc(keyboardQuadric);
	glutMainLoop();
	return 0;
}
