...
void reshapeAlpha3D(int w, int h)
{
...
void keyboardAlpha3D(unsigned char key, int x, int y)
{
   switch (key) {
      case 'a':
      case 'A':
...
int mainAlpha3D(int argc, char** argv)
{
   ...
   initAlpha3D();
   glutReshapeFunc(reshapeAlpha3D);
   glutKeyboardFunc(keyboardAlpha3D);
   glutDisplayFunc(displayAlpha3D);
   glutMainLoop();
   return 0;
}
