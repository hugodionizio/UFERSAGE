...
void reshapePolyOff(int width, int height)
{
...
void mousePolyOff(int button, int state, int x, int y) {
	switch (button) {
...
void keyboardPolyOff (unsigned char key, int x, int y)
{
	switch (key) {
		case 't':
		if (tdist < 4.0) {
 ...
 */
int mainPolyOff(int argc, char** argv)
{
	glutInit(&argc, argv);
	glutInitDisplayMode(GLUT_SINGLE | GLUT_RGB | GLUT_DEPTH);
	glutCreateWindow(argv[0]);
	glutReshapeFunc(reshapePolyOff);
	glutDisplayFunc(displayPolyOff);
	glutMouseFunc(mousePolyOff);
	glutKeyboardFunc(keyboardPolyOff);
	gfxinit();
	glutMainLoop();
	return 0;
}
