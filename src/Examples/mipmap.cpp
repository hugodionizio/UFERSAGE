...
void reshapeMipMap(int w, int h)
{
...
void keyboardMipMap (unsigned char key, int x, int y)
{
   switch (key) {
      case 27:
         exit(0);
...
int mainMipMap(int argc, char** argv)
{
   ...
   initMipMap ();
   glutDisplayFunc(displayMipMap);
   glutReshapeFunc(reshapeMipMap);
   glutKeyboardFunc(keyboardMipMap);
   glutMainLoop();
   return 0;     
}
